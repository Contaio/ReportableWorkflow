...
public class ReferenceChecker {
	...
	protected static final boolean checkIncomingReferences(final Handle handle, final Manager manager) {
		boolean result = true;
		...
		if (Action.DELETE.equals(manager.getAction()) && handle.getKeyObject() instanceof IDProvider) {
			for (ReferenceEntry referenceEntry : ((StoreElement) handle.getKeyObject()).getIncomingReferences()) {
				...
				Logging.logInfo(String.format("Handle (%s) has %d incoming references.", handleName, ((IDProvider) handle.getKeyObject()).getIncomingReferences().length), ReferenceChecker.class);
				if (isActionableReferenceEntry(referenceEntry)) {
					...
					if (handle.getPreviousHandle() != null && referenceEntry.getReferencedObject().equals(handle.getPreviousHandle().getKeyObject())) {
						Logging.logInfo("the reference is the previous handle (possible parent object), it gets ignored.", ReferenceChecker.class);
						continue;
					}

					// Checks whether the element is (one of) the initial element(s) to be deleted.
					Logging.logInfo("Check whether the element is (one of) the initial element(s) to be deleted.", ReferenceChecker.class);
					...
					if (startStoreElement != null) {
						...
						Logging.logInfo("Element is not one of the initial elements to be deleted.", ReferenceChecker.class);
						manager.getStartStoreElements().forEach(el -> {
							Logging.logInfo("\t - " + el.getName(), ReferenceChecker.class);
						});
					}
				}
			}
	...
	protected static final Set<StoreElement> checkOutgoingReferences(final Handle handle, final Manager manager) {
		Set<StoreElement> referencedElementsSet = new HashSet<>();
			...
			for (ReferenceEntry referenceEntry : ((StoreElement) handle.getKeyObject()).getOutgoingReferences()) {
				if (!referenceEntry.getRelease()) {
					if (referenceEntry.isBroken()) {
						...
						referencedElementsSet.add(((StoreElement) handle.getKeyObject()));
						...
						if (storeElement == null) {
							if (!hasUserPermissions(manager, (StoreElement) handle.getKeyObject())) {
								ResolveObject resolveObject = new ResolveObject(new ReferencedEntryHandle(((StoreElement) handle.getKeyObject()), handle, manager, referenceEntry, ReferencedEntryHandle.Type.MISSING_PERMISSION), handle);
								handle.addNextHandleObject(resolveObject);
							} else {
							...
							if (addStoreElement) {
								...
								handle.addNextHandleObject(resolveObject);
								referencedElementsSet.add(storeElement);
							}
						}
					}
				}
		...
		return referencedElementsSet;
	}


	private static IDProvider getReferencedIdProvider(final ReferenceEntry referenceEntry) {
	...
	private static boolean hasUserPermissions(final Manager manager, final StoreElement storeElement) {
		final UserAgent userAgent = manager.getContext().requestSpecialist(UserAgent.TYPE);
		if (Objects.isNull(userAgent)) {
			return false;
		}
		final User user = userAgent.getUser();
		// check permission
		switch (manager.getAction()) {
			case RELEASE:
				return storeElement.getPermission(user).canRelease();
			case DELETE:
				return storeElement.getPermission(user).canDelete();
			default:
				return false;
		}
	}
