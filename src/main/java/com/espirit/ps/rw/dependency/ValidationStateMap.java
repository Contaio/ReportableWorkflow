...
public class ValidationStateMap {
					...
					switch (manager.getAction()) {
						case RELEASE:
							if (((IDProvider) element).isReleaseSupported() && !resultList.contains(element)) {
								...
								if (!((IDProvider) element).isReleased()) {
									List<ValidationState> validationStates = validationMap.get(element);
									shouldBeIgnored = validationStates.stream().anyMatch(state -> state.getType().equals(ValidationState.Type.IN_RELEASE_STORE));
								}
						...
						case DELETE:
							if (!resultList.contains(element)) {
								...
								List<ValidationState> validationStates = validationMap.get(element);
								boolean               shouldBeIgnored  = validationStates.stream().anyMatch(state -> state.getType().equals(ValidationState.Type.IGNORED_ON_DELETE));
								if (!shouldBeIgnored) {
									resultList.add((IDProvider) element);
								}
							}
	...
	public List<IDProvider> getActionableElements(final boolean onlyWithStates) {
		List<IDProvider> resultList = new LinkedList<>();
	...
	public ValidationStateList getAllValidationStates(final ValidationState.Type... without) {
		...
		if (Objects.isNull(without) || without.length == 0) {
			validationMap.values().stream().forEach(resultList::addAll);
		} else {
			validationMap.entrySet().stream().flatMap(e -> e.getValue().stream()).filter(e -> {
				for (final ValidationState.Type type : without) {
