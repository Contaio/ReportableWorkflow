...
public class ClientSession {
	...
	public static Object getAndDeleteItem(final SpecialistsBroker broker, final String name, final Object fallback) {
		...
		if (cientSession == null) {
			...
			return fallback;
		} else if (!cientSession.hasItem(name)) {
			AdvancedLogger.logInfo("Client session has no items.", ClientSession.class);
			return fallback;
		} else {
			Object item = cientSession.getItem(name);
	...
	public static IDProvider getElement(final SpecialistsBroker broker) {
		AdvancedLogger.logInfo("Fetching Element", ClientSession.class);
		...
		if (webeditUiAgent == null || isTPPSupported(broker)) {
			AdvancedLogger.logInfo("Fetching by clientSession: " + LogHelper.getTextIdentification(clientSession != null ? clientSession.getElement() : null),
								   ClientSession.class);
			return clientSession != null ? clientSession.getElement() : null;
		} else {
			AdvancedLogger.logInfo("Fetching by webeditUiAgent: " + LogHelper.getTextIdentification(webeditUiAgent.getPreviewElement()),
								   ClientSession.class);
	...
	public static Object getItem(final SpecialistsBroker broker, final String name, final Object fallback) {
		AdvancedLogger.logInfo("Fetching item", ClientSession.class);
		ClientSession clientSession = getClientSession(broker);
		if (clientSession != null) {
			AdvancedLogger.logInfo(clientSession.hasItem(name) ? ("Retrieveing:" + LogHelper.getTextIdentification(clientSession.getItem(name)))
										   : ("Fallback: " + fallback), ClientSession.class);
			return clientSession.hasItem(name) ? clientSession.getItem(name) : fallback;
		} else {
			AdvancedLogger.logInfo("Client Session null. Fallback:" + LogHelper.getTextIdentification(fallback), ClientSession.class);
			return fallback;
		}
	}
